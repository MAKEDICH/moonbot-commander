...
echo.
echo  ===================================================================
echo  SERVER SETUP COMPLETE! Now select [4] to start PRODUCTION
echo  ===================================================================
echo.
pause
goto MENU
...
cd /d "%SCRIPT_DIR%backend"
python -X utf8 utils/fix_env_encoding.py >nul 2>&1
python -X utf8 utils/set_server_mode.py local >nul 2>&1
echo        [OK] MOONBOT_MODE=local configured
cd /d "%SCRIPT_DIR%"
...
start "MoonBot-Backend-DEV" cmd /c "cd /d %PDIR%\backend && set MOONBOT_MODE=local && set PYTHONUTF8=1 && python -X utf8 -m uvicorn main:app --host 0.0.0.0 --port 8000 --reload"
timeout /t 3 /nobreak >nul
echo        [OK] Backend started

echo  [5/5] Starting Frontend...
start "MoonBot-Scheduler-DEV" cmd /c "cd /d %PDIR%\backend && set MOONBOT_MODE=local && set PYTHONUTF8=1 && python -X utf8 -m services.scheduler"
timeout /t 2 /nobreak >nul
...
cd /d "%SCRIPT_DIR%backend"
python -X utf8 utils/fix_env_encoding.py >nul 2>&1
python -X utf8 utils/set_server_mode.py >nul 2>&1
echo        [OK] MOONBOT_MODE=server configured
cd /d "%SCRIPT_DIR%"
...
set "PDIR=%SCRIPT_DIR:~0,-1%"
start "MoonBot-Backend-PROD" cmd /c "cd /d %PDIR%\backend && set MOONBOT_MODE=server && set PYTHONUTF8=1 && python -X utf8 -m uvicorn main:app --host 0.0.0.0 --port 8000 --workers 1 --limit-concurrency 2000 --backlog 4096 --timeout-keep-alive 30"
timeout /t 4 /nobreak >nul
echo        [OK] Backend started

start "MoonBot-Scheduler" cmd /c "cd /d %PDIR%\backend && set MOONBOT_MODE=server && set PYTHONUTF8=1 && python -X utf8 -m services.scheduler"
timeout /t 2 /nobreak >nul
...
echo.
echo  Configuration:
echo    - Workers: 1 (required for UDP)
echo    - Mode: server
