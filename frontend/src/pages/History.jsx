...
import { FiClock, FiServer, FiTrash2, FiCheckCircle, FiXCircle, FiSearch, FiCopy, FiSend, FiRotateCcw } from 'react-icons/fi';
...
const History = () => {
  ...
  const [sendingRevert, setSendingRevert] = useState(null); // ID команды которая отправляется
  const [copiedAction, setCopiedAction] = useState(null); // {id: itemId, type: 'forward'|'revert'}
  const [copiedBtn, setCopiedBtn] = useState(null); // Для показа галочки после копирования (формат: "forward-ID" или "revert-ID")
  ...
  const handleClearHistory = async () => {
    const confirmed = await confirm({
      ...
      message: 'Вы уверены, что хотите очистить историю команд?',
      type: 'danger',
  ...
  // Проверяем, является ли команда SetParam
  const isSetParamCommand = (command) => {
    return command.toLowerCase().startsWith('setparam ');
  };

  // Парсим SetParam команду: SetParam "StrategyName" ParamName Key=Value ...
  const parseSetParamCommand = (command) => {
    // Формат: SetParam "StrategyName" ParamName Key1=Value1 Key2=Value2 ...
    const match = command.match(/^SetParam\s+"([^"]+)"\s+(\w+)\s+(.+)$/i);
    if (!match) return null;
    
    return {
      strategyName: match[1],
      paramName: match[2],
      params: match[3] // Например: "43=4 50=300"
    };
  };

  // Создать revert команду (заменяем значения на ???)
  const createRevertCommand = (command) => {
    const parsed = parseSetParamCommand(command);
    if (!parsed) return command;
    
    // Заменяем значения в парах key=value на key=???
    const revertParams = parsed.params.replace(/(\d+)=[\d.]+/g, '$1=???');
    return `SetParam "${parsed.strategyName}" ${parsed.paramName} ${revertParams}`;
  };
  ...
  // Копировать команду как есть
  const handleCopyForward = (command, itemId) => {
    copyToClipboard(command);
    setCopiedBtn(`forward-${itemId}`);
    setTimeout(() => setCopiedBtn(null), 2000); // Убираем галочку через 2 сек
  };

  // Копировать revert команду
  const handleCopyRevert = (command, itemId) => {
    const revertCmd = createRevertCommand(command);
    copyToClipboard(revertCmd);
    setCopiedBtn(`revert-${itemId}`);
    setTimeout(() => setCopiedBtn(null), 2000);
  };

  // Отправить Revert команду на сервер
  const handleSendRevert = async (item) => {
    const revertCmd = createRevertCommand(item.command);
    
    // Проверяем что есть значения для отправки (не ???)
    if (revertCmd.includes('???')) {
      showError('Сначала скопируйте Revert команду, замените ??? на нужные значения, затем отправьте через раздел Команды');
      return;
    }
    
    const confirmed = await confirm({
      title: 'Отправить Revert команду',
      message: `Отправить команду на сервер "${getServerName(item.server_id)}"?\n\n${revertCmd}`,
      type: 'warning',
    ...
    setSendingRevert(item.id);
    try {
      await commandsAPI.send({
        server_id: item.server_id,
        command: revertCmd
      });
      setSendingRevert(`done-${item.id}`);
      setTimeout(() => setSendingRevert(null), 2000); // Показываем галочку 2 сек
      await loadData(); // Обновляем историю
    } catch (error) {
      showError('Ошибка отправки команды: ' + (error.response?.data?.detail || error.message));
      setSendingRevert(null);
    }
  };
            ...
            <div key={item.id} className={styles.historyItem}>
              <div className={styles.itemHeader}>
                ...
                <div className={styles.itemTime}>
                  {formatServerDateTime(item.execution_time)}
                </div>
              ...
              <div className={styles.itemCommand}>
                <div className={styles.commandHeader}>
                  ...
                  {isSetParamCommand(item.command) && (
                    <div className={styles.commandActions}>
                      <button 
                        className={`${styles.actionBtn} ${copiedBtn === `forward-${item.id}` ? styles.copied : ''}`}
                        onClick={() => handleCopyForward(item.command, item.id)}
                        title="Копировать команду"
                      >
                        {copiedBtn === `forward-${item.id}` ? <><FiCheckCircle /> Copied!</> : <><FiCopy /> Copy Forward</>}
                      </button>
                      <button 
                        className={`${styles.actionBtn} ${styles.revertBtn} ${copiedBtn === `revert-${item.id}` ? styles.copied : ''}`}
                        onClick={() => handleCopyRevert(item.command, item.id)}
                        title="Копировать Revert (значения заменены на ???)"
                      >
                        {copiedBtn === `revert-${item.id}` ? <><FiCheckCircle /> Copied!</> : <><FiRotateCcw /> Copy Revert</>}
                      </button>
                      <button 
                        className={`${styles.actionBtn} ${styles.sendBtn} ${sendingRevert === `done-${item.id}` ? styles.sent : ''}`}
                        onClick={() => handleSendRevert(item)}
                        disabled={sendingRevert === item.id}
                        title="Отправить Revert команду на этот сервер"
                      >
                        {sendingRevert === item.id ? <><FiSend /> Sending...</> : 
                         sendingRevert === `done-${item.id}` ? <><FiCheckCircle /> Sent!</> : 
                         <><FiSend /> Send Revert</>}
                      </button>
                    </div>
                  )}
