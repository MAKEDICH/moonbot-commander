...
import { parseServerTimeToMs } from '../../utils/dateUtils';
...
export const useChartData = (chartData) => {
  ...
  const sortedClosestPrices = useMemo(() => {
    ...
    return closest_prices
      .map(cp => ({
        timeMs: parseServerTimeToMs(cp.time),
        price: cp.price,
      }))
      .filter(cp => !isNaN(cp.timeMs))
  ...
  const priceData = useMemo(() => {
    ...
    return dataToProcess.map((p, index) => {
      const timeMs = parseServerTimeToMs(p.time);
  ...
  const volumeData = useMemo(() => {
    ...
    return candles.map(candle => {
      const timeMs = parseServerTimeToMs(candle.time);
  ...
  const { buyTrades, sellTrades } = useMemo(() => {
    ...
    if (trades?.length > 0) {
      trades.forEach((trade) => {
        const tradeTimeMs = parseServerTimeToMs(trade.time);
        if (isNaN(tradeTimeMs)) return;
  ...
  // Функция парсинга времени ордера (без конвертации часового пояса)
  const parseOrderTime = useCallback((timeStr) => {
    if (!timeStr) return NaN;
    const normalized = timeStr.replace(' ', 'T');
    if (normalized.includes('-')) {
      return parseServerTimeToMs(normalized);
    }
    if (baseDate) {
      return parseServerTimeToMs(`${baseDate}T${normalized}`);
    }
    return parseServerTimeToMs(normalized);
  }, [baseDate]);
