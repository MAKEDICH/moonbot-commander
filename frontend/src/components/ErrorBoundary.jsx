 ...
 * 
 * РАЗМЫШЛЕНИЕ: ErrorBoundary работает как try-catch для React компонентов.
 * Ловит ошибки в:
 * - render() методе
 * - lifecycle методах
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(error, errorInfo) {
    // РАЗМЫШЛЕНИЕ: Здесь можно отправить ошибку в сервис логирования (Sentry, LogRocket)
    ...
    // TODO: Отправить в систему мониторинга ошибок
    // if (window.Sentry) {
  ...
  handleReset = () => {
    // РАЗМЫШЛЕНИЕ: Перезагрузка страницы - простое но эффективное решение
    // Альтернатива - сбросить state и попытаться re-render
    window.location.reload();
  };

  render() {
    if (this.state.hasError) {
      // РАЗМЫШЛЕНИЕ: Показываем user-friendly сообщение вместо белого экрана
      return (
        <div style={{
          display: 'flex',
          ...
          <div style={{
            ...
            {/* РАЗМЫШЛЕНИЕ: Показываем детали ошибки только в development */}
