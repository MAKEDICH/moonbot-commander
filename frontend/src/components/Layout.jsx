...
import { FiHome, FiServer, FiCommand, FiClock, FiLogOut, FiUsers, FiKey, FiCalendar, FiHeart, FiCopy, FiCheck, FiTrendingUp, FiMenu, FiX, FiDollarSign, FiDownload, FiShield, FiBookOpen } from 'react-icons/fi';
...
const Layout = () => {
  ...
  // Проверка обновлений при загрузке и каждые 30 минут
  useEffect(() => {
    const checkForUpdates = async () => {
      try {
        const response = await api.get('/api/updates/check');
        setUpdateAvailable(response.data?.update_available === true);
      } catch (err) {
        // Игнорируем ошибки - не критично для UI
  ...
  const navItems = [
    ...
    { path: '/updates', icon: <FiDownload />, label: 'Обновления', badge: updateAvailable },
    { path: '/backup', icon: <FiShield />, label: 'Бэкап' },
  ];
        ...
        <nav className={styles.nav}>
          {navItems.map((item) => (
            ...
            >
              ...
              {item.badge && <span className={styles.updateBadge} />}
            </NavLink>
          ))}
        </nav>

        {/* Блок документации */}
        <NavLink to="/docs" className={styles.docsSection} onClick={handleNavClick}>
          <FiBookOpen className={styles.docsIcon} />
          <div className={styles.docsText}>
            <span className={styles.docsTitle}>Документация</span>
            <span className={styles.docsSubtitle}>Руководство пользователя</span>
          </div>
        </NavLink>
