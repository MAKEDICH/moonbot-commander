...
from backup_service import BackupService, create_pre_upgrade_backup
...
class MigrationManager:
    ...
    def _is_database_locked(self) -> bool:
        ...
        РАЗМЫШЛЕНИЕ: SQLite имеет exclusive lock при write операциях.
        Если main.py запущен и держит соединение → миграция упадет!
        Проверяем это ДО начала миграции.
    ...
    def migrate_to_v2(self, create_backup: bool = True) -> bool:
        ...
        try:
            ...
            # ИСПРАВЛЕНО: Проверка что БД не используется другим процессом
            # РАЗМЫШЛЕНИЕ: SQLite не поддерживает параллельные write операции!
            # Если main.py запущен → exclusive lock → миграция упадет!
            ...
            try:
                ...
                # РАЗМЫШЛЕНИЕ: Не игнорируем ошибку, а анализируем её!
    ...
    def _verify_integrity(self) -> bool:
        ...
        РАЗМЫШЛЕНИЕ: Просто count() недостаточно!
        Нужно проверить:
        - Нет ли NULL в обязательных полях
    ...
    def rollback_migration(self, backup_path: str) -> bool:
        ...
        # ИСПРАВЛЕНО: Проверяем бэкап ПЕРЕД восстановлением
        # РАЗМЫШЛЕНИЕ: Что если файл не существует? Или поврежден?
        # Восстановление испорченного бэкапа → потеря всех данных!
