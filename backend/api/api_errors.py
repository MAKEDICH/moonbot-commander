...
def _check_table_exists(db: Session) -> bool:
    """Проверить существует ли таблица moonbot_api_errors"""
    from sqlalchemy import text
    try:
        db.execute(text("SELECT 1 FROM moonbot_api_errors LIMIT 1"))
        return True
    except OperationalError:
        db.rollback()  # Rollback после ошибки
        return False
...
async def get_api_errors(
    ...
    if not _check_table_exists(db):
        log("[API-ERRORS] Table moonbot_api_errors does not exist yet")
        return []
