...
async def get_udp_listener_status(
    ...
    # Получаем статус из памяти (может быть пустым при нескольких workers)
    runtime_status: Dict[str, Any] = udp.get_listener_status(server_id)
    
    # Получаем статус из БД (primary source of truth)
    ...
    # - Если в памяти есть listener - используем его статус
    # - Иначе читаем из БД (для случаев с несколькими workers или после рестарта)
    is_running = runtime_status["is_running"]
    if not is_running and db_status:
        is_running = db_status.is_running
    
    messages_received = runtime_status["messages_received"]
    if messages_received == 0 and db_status:
        messages_received = db_status.messages_received or 0
    ...
    return {
        ...
        "last_error": runtime_status["last_error"] or (db_status.last_error if db_status else None),
        "db_status": {
            "started_at": format_iso(db_status.started_at) if db_status else None,
            "last_message_at": format_iso(db_status.last_message_at) if db_status else None,
