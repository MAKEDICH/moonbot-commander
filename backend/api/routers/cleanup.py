...
async def get_cleanup_stats(
    ...
    stats = cleanup_service.get_database_stats(current_user.id, db)
    return stats
...
async def get_cleanup_settings(
    ...
    return {
        ...
        'last_cleanup': settings.last_cleanup.isoformat() if settings.last_cleanup else None
    }
...
async def cleanup_logs(
    ...
    result = cleanup_service.cleanup_old_logs(current_user.id, action_data.days, db)
    return result
...
async def cleanup_history(
    ...
    result = cleanup_service.cleanup_command_history(current_user.id, action_data.days, db)
    return result
...
async def vacuum_db(
    ...
    result = cleanup_service.vacuum_database(db)
    return result
...
async def cleanup_backend_logs(
    ...
    result = cleanup_service.cleanup_backend_logs(action_data.max_size_mb)
    return result
...
async def full_cleanup(
    ...
    result = cleanup_service.full_cleanup(current_user.id, db)
    return result
