...
def _get_engine_config() -> Dict[str, Any]:
    ...
    if is_sqlite:
        # SQLite конфигурация
        # Работает для 3000+ серверов благодаря WAL mode и оптимизациям
        # Для максимальной производительности рекомендуется PostgreSQL
        ...
        log("[DATABASE] Using SQLite (optimized: WAL mode, 64MB cache)")
...
# expire_on_commit=False предотвращает проблемы при параллельных запросах
# когда объект может быть использован после commit в другом потоке
