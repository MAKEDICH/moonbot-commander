...
async def _init_high_load_components() -> None:
    ...
    # Логируем режим работы (читаем дефолт из YAML конфига)
    ...
    # Применение миграции для log_categories (выборочное логирование)
    try:
        from updates.migrate_add_log_categories import migrate as migrate_log_categories
        if migrate_log_categories():
            log("[STARTUP] ✅ log_categories migration applied")
        else:
            log("[STARTUP] ✅ log_categories already exists")
    except Exception as e:
        log(f"[STARTUP] log_categories migration skipped: {e}", level="DEBUG")
    
    # Исправление временных меток ордеров (UTC+3 -> локальное время)
    try:
        from updates.migrate_fix_order_timestamps import (
            check_migration_needed as check_timestamps_migration,
            run_migration as run_timestamps_migration
        )
        if check_timestamps_migration():
            log("[STARTUP] Applying order timestamps fix migration...")
            run_timestamps_migration()
            log("[STARTUP] ✅ Order timestamps fixed")
        else:
            log("[STARTUP] ✅ Order timestamps already fixed")
    except Exception as e:
        log(f"[STARTUP] Order timestamps migration skipped: {e}", level="DEBUG")
    
    # Создание таблицы moonbot_api_errors
    try:
        from updates.migrate_add_api_errors_table import (
            check_migration_needed as check_api_errors_migration,
            run_migration as run_api_errors_migration
        )
        if check_api_errors_migration():
            log("[STARTUP] Creating moonbot_api_errors table...")
            run_api_errors_migration()
            log("[STARTUP] ✅ moonbot_api_errors table created")
        else:
            log("[STARTUP] ✅ moonbot_api_errors table exists")
    except Exception as e:
        log(f"[STARTUP] moonbot_api_errors migration skipped: {e}", level="DEBUG")
    
    # Добавление колонки last_errors_viewed_at
    try:
        from updates.migrate_add_last_errors_viewed import (
            check_migration_needed as check_last_errors_migration,
            run_migration as run_last_errors_migration
        )
        if check_last_errors_migration():
            log("[STARTUP] Adding last_errors_viewed_at column...")
            run_last_errors_migration()
            log("[STARTUP] ✅ last_errors_viewed_at column added")
        else:
            log("[STARTUP] ✅ last_errors_viewed_at column exists")
    except Exception as e:
        log(f"[STARTUP] last_errors_viewed_at migration skipped: {e}", level="DEBUG")
...
async def startup_handler() -> None:
    ...
    ВАЖНО: Рекомендуется запуск с --workers 1 для корректной работы UDP.
    """
    ...
    db = SessionLocal()
    try:
        # Диагностика: показываем количество серверов
        ...
        if len(all_servers) <= 10:
            for s in all_servers:
                log(f"[STARTUP] Server ID={s.id}, Name={s.name}, Active={s.is_active}")
        else:
    ...
    # Scheduler запускается отдельным процессом через MOONBOT.bat
    log("[STARTUP] Scheduler runs as separate process (see MoonBot-Scheduler window)")
