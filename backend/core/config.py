...
# Устанавливаем кодировку для чтения .env файлов на Windows
# Это нужно ДО импорта slowapi который читает .env через starlette.config
...
# Используем in-memory storage чтобы избежать чтения .env файла с русскими символами
