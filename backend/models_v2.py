...
class User(Base):
    ...
    servers = relationship("Server", back_populates="owner", cascade="all, delete-orphan", lazy="selectin")
    command_history = relationship("CommandHistory", back_populates="user", cascade="all, delete-orphan", lazy="dynamic")
    quick_commands = relationship("QuickCommand", back_populates="user", cascade="all, delete-orphan", lazy="selectin")
    command_presets = relationship("CommandPreset", back_populates="user", cascade="all, delete-orphan", lazy="selectin")
    user_settings = relationship("UserSettings", back_populates="user", uselist=False, cascade="all, delete-orphan", lazy="selectin")
    recovery_codes = relationship("RecoveryCode", back_populates="user", cascade="all, delete-orphan", lazy="selectin")
    scheduled_commands = relationship("ScheduledCommand", back_populates="user", cascade="all, delete-orphan", lazy="dynamic")
...
class Server(Base):
    ...
    command_history = relationship("CommandHistory", back_populates="server", cascade="all, delete-orphan", lazy="dynamic")
    server_status = relationship("ServerStatus", back_populates="server", uselist=False, cascade="all, delete-orphan", lazy="selectin")
...
class TwoFactorAttempt(Base):
    ...
    ip_address = Column(String(45), nullable=True, index=True)  # IPv6 поддержка
...
class ScheduledCommandServer(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    scheduled_command_id = Column(Integer, ForeignKey("scheduled_commands.id", ondelete="CASCADE"), nullable=False, index=True)
    server_id = Column(Integer, ForeignKey("servers.id", ondelete="CASCADE"), nullable=False, index=True)
    
    __table_args__ = (
        Index('idx_sched_server_link', 'scheduled_command_id', 'server_id'),
        UniqueConstraint('scheduled_command_id', 'server_id', name='uq_sched_server'),
    )
...
class CommandImage(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    command_history_id = Column(Integer, ForeignKey("command_history.id", ondelete="CASCADE"), nullable=False, unique=True, index=True)
...
class MoonBotOrder(Base):
    ...
    __table_args__ = (
        Index('idx_order_server_moonbot', 'server_id', 'moonbot_order_id', unique=True),
...
class DataBackup(Base):
    ...
    backup_type = Column(String(20), default="manual")  # manual, automatic, pre-upgrade
