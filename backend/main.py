...
# ВАЖНО: Устанавливаем UTF-8 кодировку ДО любых импортов
# Это исправляет UnicodeDecodeError на Windows при чтении .env с русскими комментариями
os.environ.setdefault('PYTHONUTF8', '1')
if sys.platform == 'win32':
    # Дополнительно устанавливаем кодировку для stdin/stdout
    # Проверяем что stdout ещё не был заменён (защита от циклических импортов)
    import codecs
...
# Загружаем .env ПЕРВЫМ (до любых импортов которые могут его загрузить)
# override=False означает что переменные из командной строки (set MOONBOT_MODE=server) 
# имеют приоритет над .env файлом
...
# Логируем текущий режим при старте
current_mode = os.getenv('MOONBOT_MODE', 'server')
print(f"[MAIN] MOONBOT_MODE = {current_mode}")
