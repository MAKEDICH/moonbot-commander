...
class SQLParserUpdateMixin:
    ...
    def _apply_update_fields(self, order: models.MoonBotOrder, updates: dict):
        ...
        field_mapping = {
            ...
            # Status обрабатывается отдельно (не сохраняем в БД)
        }
        ...
        # ВАЖНО: Status=1 от MoonBot - ЯВНЫЙ сигнал закрытия ордера!
        # Это самый надёжный признак - MoonBot явно говорит что ордер закрыт
        if 'Status' in updates and utils.safe_int(updates['Status']) == 1:
            if order.status != "Closed":
                order.status = "Closed"
                if not order.closed_at:
                    order.closed_at = utcnow()
                log(f"[UDP-LISTENER-{self.server_id}] ✅ Order {order.moonbot_order_id} marked as Closed (MoonBot Status=1)")
