    ...
    def process_lst_response(self, message: str):
        ...
        try:
            ...
            try:
                ...
                # УБРАНО: Автозакрытие ордеров при несовпадении количества
                # Ордера теперь остаются Open до получения явного CloseDate
                # Это позволяет пользователям видеть на каком боте остались открытые ордера
                if our_open_count > total_open:
                    excess = our_open_count - total_open
                    log(f"[UDP-LISTENER-{self.server_id}] ℹ️ LST shows {total_open} open, we have {our_open_count} ({excess} extra) - NOT auto-closing, waiting for explicit CloseDate")
