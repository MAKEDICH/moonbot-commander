...
from utils.datetime_utils import utcnow, timestamp_to_datetime
from . import utils
...
class SQLParserInsertMixin:
    ...
    def parse_insert_order(self, db: Session, sql: str, command_id: int, moonbot_order_id: int = None, bot_name: str = None):
        ...
        try:
            ...
            # Сохраняем bot_name - из пакета или fallback на имя сервера
            if bot_name:
                order.bot_name = bot_name
            elif not order.bot_name:
                # Fallback: используем имя сервера из БД
                server = db.query(models.Server).filter(models.Server.id == self.server_id).first()
                if server and server.name:
                    order.bot_name = server.name
                    log(f"[UDP-LISTENER-{self.server_id}] 💾 Set bot_name from server name for order {moonbot_order_id}: {server.name}")
    ...
    def _apply_insert_fields(self, order: models.MoonBotOrder, data: dict, db: Session):
        ...
        field_mapping = {
            ...
            # 'Status': пропускаем - обрабатывается отдельно через умную логику
            'Comment': 'comment',
        ...
        # Определение статуса по полю Status из MoonBot
    ...
    def _process_insert_close_logic(self, order: models.MoonBotOrder, data: dict):
        """
        Обработка статуса при INSERT
        
        ЛОГИКА (согласно документации MoonBot):
        1. Проверяем поле Status из данных MoonBot
        2. Status=1 → ордер закрыт
        3. Status=0 или отсутствует → ордер открыт
        
        НЕ используем SellPrice/ProfitBTC/GainedBTC как индикаторы,
        т.к. MoonBot может присылать их уже при открытии ордера.
        """
        from . import utils
        
        # Проверяем Status из данных MoonBot (это официальный признак)
        status_value = utils.safe_int(data.get('Status', 0))
        
        if status_value == 1:
            # Status=1 означает ордер закрыт
            order.status = "Closed"
            if not order.closed_at:
                order.closed_at = utcnow()
            log(f"[UDP-LISTENER-{self.server_id}] [INSERT] Order {order.moonbot_order_id} has Status=1 → Closed")
        else:
            # Status=0 или отсутствует → ордер открыт
            order.status = "Open"
            order.closed_at = None
            if not order.opened_at:
                order.opened_at = utcnow()
            log(f"[UDP-LISTENER-{self.server_id}] [INSERT] Order {order.moonbot_order_id} has Status={status_value} → Open")
