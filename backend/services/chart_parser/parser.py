...
def parse_chart_binary(data: bytes) -> Optional[ChartData]:
    ...
    try:
        ...
        # 2. Исторические цены - ВАЖНО: порядок (Price, Time)!
        n_history = reader.read_int()
        for _ in range(n_history):
            price = reader.read_double()  # Сначала price!
            time = reader.read_datetime()  # Потом time!
        ...
        # 4. Трейды - порядок (Time, Price)
        n_trades = reader.read_int()
        for _ in range(n_trades):
            time = reader.read_datetime()
            price = reader.read_double()  # Отрицательная = продажа
        ...
        # 6. Линия средней цены - порядок (Price, Time)
