...
CACHE_FILE_PATH = BASE_DIR / "update_check_cache.json"
...
class UpdateChecker:
    ...
    def _get_current_version(self) -> str:
        ...
        for path in search_paths:
            try:
                if path.exists():
                    # Читаем как байты для обработки BOM
                    raw_bytes = path.read_bytes()
                    
                    # Определяем кодировку по BOM
                    if raw_bytes.startswith(b'\xff\xfe'):
                        # UTF-16 LE BOM
                        version_value = raw_bytes.decode('utf-16').strip()
                    elif raw_bytes.startswith(b'\xfe\xff'):
                        # UTF-16 BE BOM
                        version_value = raw_bytes.decode('utf-16').strip()
                    elif raw_bytes.startswith(b'\xef\xbb\xbf'):
                        # UTF-8 BOM
                        version_value = raw_bytes[3:].decode('utf-8').strip()
                    else:
                        # UTF-8 без BOM
                        version_value = raw_bytes.decode('utf-8').strip()
                    
                    # Убираем пробелы между символами (защита от повреждённых файлов)
                    version_value = version_value.replace(' ', '').replace('\t', '')
                    
                    return version_value.replace('v', '')
            except Exception as e:
                logger.warning(f"Не удалось прочитать версию из {path}: {e}")
    ...
    def _load_cache(self) -> Optional[Dict]:
        ...
        try:
            ...
            pass  # Кэш повреждён или недоступен
        return None
    ...
    async def check_for_updates(self, force: bool = False) -> Optional[Dict]:
        ...
        try:
            ...
            if not force:
                ...
                if cache:
                    try:
                        last_check_str = cache.get('last_check', '2000-01-01')
                        last_check = datetime.fromisoformat(last_check_str)
                        
                        # Приводим обе даты к naive datetime для сравнения
                        now_naive = utcnow().replace(tzinfo=None)
                        last_check_naive = last_check.replace(tzinfo=None) if last_check.tzinfo else last_check
                        
                        if now_naive - last_check_naive < timedelta(seconds=self.CHECK_INTERVAL):
                            # Используем кэш
                            self.update_available = cache.get(
                                'update_available', False)
                            self.latest_version_info = cache.get(
                                'latest_version_info')
                            return self.latest_version_info if self.update_available else None
                    except (ValueError, TypeError) as e:
                        logger.warning(f"Ошибка парсинга даты кэша: {e}")
